...
import { granularidadeParaJanela, serieTemporal } from "@/lib/metrics/timeseries";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const granularidade = granularidadeParaJanela(
      periodo.to.getTime() - periodo.from.getTime(),
    );
    const data = await serieTemporal(
    ...
    return metricsError("internal", "Não foi possível calcular a série temporal");
  }
}
