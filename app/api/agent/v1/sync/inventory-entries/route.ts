...
import { inventoryEntriesPayloadSchema } from "@/lib/agent-cloud/agent-payload-schemas";
...
export async function POST(request: Request) {
  const auth = await authenticatedAgentPayload(
    request,
    inventoryEntriesPayloadSchema,
  );
  ...
  try {
    return NextResponse.json(
      await ingestAgentBatch({
        ...
        dataType: "inventory_entries_daily",
