...
/** Horários de encerramento usados na rotina da loja. */
const CLOSING_TIMES: Array<[number, number, string]> = [
  [12, 0, '12:00'],
  [18, 0, '18:00'],
  [20, 0, '20:00'],
  [22, 0, '22:00'],
...
export function QueuePromotionManager({ queueId, promotions, published, onChanged, timeZone }: { queueId: string; promotions: QueuePromotion[]; published: boolean; onChanged: () => Promise<void>; timeZone?: string }) {
  const inputRef = useRef<HTMLInputElement>(null);
  /** O período é editado e exibido no relógio da loja, o mesmo que o bot usa
   * na mensagem "Válida até" — o fuso do dispositivo do atendente não entra na conta. */
  const zone = resolveTimeZone(timeZone);
  const formatPeriod = (value: string) => formatZonedDateTime(value, zone);
  ...
  /** Duração digitada em horas. Estado próprio para o campo não brigar com a digitação
   * enquanto o valor ainda está incompleto (ex.: "1" a caminho de "12"). */
  ...
  const openNew = (source?: QueuePromotion) => {
    ...
    setEditing(null); setForm(seed); setFiles([]); setPreviewUrls((source?.media || []).map((item) => item.url)); setError(''); setCustomHours(''); setOpen(true);
  };
  const openEdit = (promotion: QueuePromotion) => { setEditing(promotion); setForm({ title: promotion.title, description: promotion.description || '', caption: promotion.caption || '', startsAt: promotion.startsAt, expiresAt: promotion.expiresAt, active: promotion.active, displayOrder: promotion.displayOrder }); setPreviewUrls((promotion.media || []).map((item) => item.url)); setFiles([]); setError(''); setCustomHours(''); setOpen(true); };
    ...
    <div className="px-6 py-5">
      <div className="flex gap-4 rounded-2xl border border-slate-200 p-3 dark:border-slate-700">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-slate-500">{formatZonedDateTime(promotion.startsAt, timeZone)} → {formatZonedDateTime(promotion.expiresAt, timeZone)}</p>
