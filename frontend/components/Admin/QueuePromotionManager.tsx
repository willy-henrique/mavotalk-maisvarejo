...
/** Troca só o horário, preservando a data já escolhida. Digitar hora dentro do
 * `datetime-local` é a parte mais custosa do formulário, e na prática a operação
 * sempre usa os mesmos horários de fechamento. */
const withTimeOfDay = (value: string, hours: number, minutes: number) => {
  const date = new Date(value);
...
const isTimeOfDay = (value: string, hours: number, minutes: number) => {
  const date = new Date(value);
  return !Number.isNaN(date.getTime()) && date.getHours() === hours && date.getMinutes() === minutes;
};

/** HH:MM local, para o campo de hora digitável. */
...
/** Aplica um "HH:MM" digitado sobre a data já escolhida. */
const withTimeString = (value: string, time: string) => {
  const [hours, minutes] = time.split(':').map((part) => Number(part));
  if (!Number.isInteger(hours) || !Number.isInteger(minutes)) return value;
  return withTimeOfDay(value, hours, minutes);
};
