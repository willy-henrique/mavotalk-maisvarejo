...
export function QueuePromotionManager({ queueId, promotions, onChanged }: { queueId: string; promotions: QueuePromotion[]; onChanged: () => Promise<void> }) {
  ...
  const closeForm = () => { setOpen(false); setEditing(null); setFile(null); setPreviewUrl(''); setError(''); };
  const openNew = (source?: QueuePromotion) => {
    ...
    setEditing(null); setForm(seed); setFile(null); setPreviewUrl(source?.media?.[0]?.url || ''); setError(''); setOpen(true);
  };
  const openEdit = (promotion: QueuePromotion) => { setEditing(promotion); setForm({ title: promotion.title, description: promotion.description || '', caption: promotion.caption || '', startsAt: promotion.startsAt, expiresAt: promotion.expiresAt, active: promotion.active, displayOrder: promotion.displayOrder }); setPreviewUrl(promotion.media?.[0]?.url || ''); setFile(null); setError(''); setOpen(true); };
  const chooseFile = (next: File | null) => {
    if (!next) return;
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(next.type) || next.size > 8 * 1024 * 1024) { setError('Selecione uma imagem JPG, PNG ou WEBP de até 8 MB.'); return; }
  ...
  const submit = async () => {
    ...
    if (!editing && !file) { setError('Selecione um flyer válido para continuar.'); return; }
    ...
    try {
      if (editing) {
        ...
        const body = new FormData(); body.append('file', file!); body.append('data', JSON.stringify(payload));
  ...
  const archive = async (promotion: QueuePromotion) => { if (!window.confirm(`Arquivar “${promotion.title}”?`)) return; setBusy(true); try { await apiPost(`/api/queues/${queueId}/promotions/${promotion.id}?action=archive`); await onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível arquivar a promoção.'); } finally { setBusy(false); } };
  ...
  return <section>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">Conteúdo publicado</p><h3 className="mt-1 text-lg font-black">Promoções e flyers</h3><p className="mt-1 text-sm text-slate-500">Organize campanhas sem misturar os dados da fila com as ofertas.</p></div><button type="button" onClick={() => openNew()} className="mavo-button-primary">+ Nova promoção</button></div>
    ...
    <div className="mt-4 space-y-3">{filtered.length ? filtered.map((promotion) => { const state = promotionState(promotion); return <article key={promotion.id} className="flex flex-col gap-4 rounded-2xl border border-slate-200 p-4 dark:border-slate-700 sm:flex-row"><img src={promotion.media?.[0]?.url || ''} alt="" className="h-24 w-full rounded-xl bg-slate-100 object-cover dark:bg-slate-800 sm:w-32" /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="font-black">{promotion.title}</h4><p className="mt-1 text-xs text-slate-500">{new Date(promotion.startsAt).toLocaleString('pt-BR')} → {new Date(promotion.expiresAt).toLocaleString('pt-BR')} · ordem {promotion.displayOrder}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${state.tone}`}>{state.label}</span></div><p className="mt-2 text-sm text-slate-500">{state.id === 'active' ? remaining(promotion.expiresAt) : promotion.caption || 'Sem legenda configurada.'}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => openEdit(promotion)} className="mavo-button-secondary min-h-0 px-3 py-2 text-xs">Editar</button><button type="button" onClick={() => openNew(promotion)} className="mavo-button-secondary min-h-0 px-3 py-2 text-xs">Duplicar</button><button type="button" disabled={busy || promotion.archived} onClick={() => void toggle(promotion)} className="mavo-button-secondary min-h-0 px-3 py-2 text-xs">{promotion.active ? 'Desativar' : 'Ativar'}</button><button type="button" disabled={busy || promotion.archived} onClick={() => void archive(promotion)} className="min-h-0 rounded-xl px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30">Arquivar</button></div></div></article>; }) : <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center dark:border-slate-700"><p className="text-lg font-black">Você ainda não possui promoções nesta visão.</p><p className="mt-2 text-sm text-slate-500">Crie uma campanha com flyer e defina quando ela estará disponível.</p><button type="button" onClick={() => openNew()} className="mavo-button-primary mt-5">Adicionar primeira promoção</button></div>}</div>
    {open && <div className="fixed inset-0 z-[calc(var(--mavo-z-modal)+1)] flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={editing ? 'Editar promoção' : 'Nova promoção'}><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"><header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5 dark:border-slate-700 dark:bg-slate-900"><div><h3 className="text-xl font-black">{editing ? 'Editar promoção' : 'Nova promoção'}</h3><p className="mt-1 text-sm text-slate-500">A promoção é salva como conteúdo da fila e só chega ao bot após publicação.</p></div><button type="button" onClick={closeForm} className="mavo-button-secondary min-h-0 px-3 py-2" disabled={busy}>Fechar</button></header><div className="grid gap-6 p-6 lg:grid-cols-[1fr_.9fr]"><div className="space-y-6"><fieldset><legend className="text-sm font-black">Informações</legend><div className="mt-3 space-y-3"><Input label="Título" value={form.title} onChange={(value) => setForm((current) => ({ ...current, title: value }))} /><Textarea label="Descrição interna" value={form.description || ''} onChange={(value) => setForm((current) => ({ ...current, description: value }))} /><Textarea label="Legenda enviada ao cliente" value={form.caption || ''} onChange={(value) => setForm((current) => ({ ...current, caption: value }))} /></div></fieldset><fieldset><legend className="text-sm font-black">Período da promoção</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><Input label="Início" type="datetime-local" value={toInputDate(form.startsAt)} onChange={(value) => setForm((current) => ({ ...current, startsAt: new Date(value).toISOString() }))} /><Input label="Expiração" type="datetime-local" value={toInputDate(form.expiresAt)} onChange={(value) => setForm((current) => ({ ...current, expiresAt: new Date(value).toISOString() }))} /></div><div className="mt-3 flex flex-wrap gap-2">{[[6, '6 horas'], [12, '12 horas'], [24, '24 horas'], [48, '48 horas'], [168, '7 dias']].map(([hours, label]) => <button key={hours} type="button" onClick={() => setDuration(hours as number)} className={`rounded-lg px-3 py-2 text-xs font-bold ${Math.abs(new Date(form.expiresAt).getTime() - new Date(form.startsAt).getTime() - Number(hours) * 3_600_000) < 60_000 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}>{label}</button>)}</div><p className="mt-3 text-xs text-slate-500">Disponível de {new Date(form.startsAt).toLocaleString('pt-BR')} até {new Date(form.expiresAt).toLocaleString('pt-BR')}.</p></fieldset><fieldset><legend className="text-sm font-black">Exibição</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><Input label="Ordem de exibição" type="number" value={String(form.displayOrder)} onChange={(value) => setForm((current) => ({ ...current, displayOrder: Number(value) || 0 }))} /><label className="flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold dark:border-slate-700"><input type="checkbox" checked={form.active} onChange={(event) => setForm((current) => ({ ...current, active: event.target.checked }))} /> Ativa ao iniciar</label></div></fieldset></div><div><p className="text-sm font-black">Flyer</p><div role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') inputRef.current?.click(); }} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); chooseFile(event.dataTransfer.files[0] || null); }} onClick={() => inputRef.current?.click()} className={`mt-3 cursor-pointer rounded-2xl border-2 border-dashed p-5 text-center transition ${dragging ? 'border-blue-500 bg-blue-50 dark:bg-blue-950/30' : 'border-slate-300 hover:border-blue-400 dark:border-slate-700'}`}><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => chooseFile(event.target.files?.[0] || null)} />{previewUrl ? <img src={previewUrl} alt="Prévia do flyer" className="mx-auto max-h-72 rounded-xl object-contain" /> : <><p className="text-lg font-black">Arraste o flyer para cá</p><p className="mt-2 text-sm text-slate-500">ou clique para selecionar uma imagem</p></>}<p className="mt-4 text-xs text-slate-500">JPG, PNG ou WEBP · máximo de 8 MB</p></div>{file && <div className="mt-3 flex items-center justify-between rounded-xl bg-slate-50 p-3 text-xs dark:bg-slate-950"><span className="truncate">{file.name} · {(file.size / 1024 / 1024).toFixed(1)} MB</span><button type="button" onClick={() => { setFile(null); setPreviewUrl(editing?.media?.[0]?.url || ''); }} className="font-bold text-rose-600">Remover</button></div>}</div></div>{error && <p role="alert" className="mx-6 mb-2 rounded-xl bg-rose-50 p-3 text-sm text-rose-700 dark:bg-rose-950/30 dark:text-rose-300">{error}</p>}<footer className="sticky bottom-0 flex justify-end gap-2 border-t border-slate-200 bg-white px-6 py-4 dark:border-slate-700 dark:bg-slate-900"><button type="button" onClick={closeForm} disabled={busy} className="mavo-button-secondary">Cancelar</button><button type="button" onClick={() => void submit()} disabled={busy} className="mavo-button-primary">{busy ? 'Salvando…' : editing ? 'Salvar promoção' : 'Adicionar promoção'}</button></footer></div></div>}
  </section>;
}
