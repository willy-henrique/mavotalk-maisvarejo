...
export function QueuePromotionManager({ queueId, promotions, published, onChanged }: { queueId: string; promotions: QueuePromotion[]; published: boolean; onChanged: () => Promise<void> }) {
  ...
  const remove = async (promotion: QueuePromotion) => {
    if (!window.confirm(`Excluir “${promotion.title}” definitivamente? Os flyers também serão removidos e a ação não pode ser desfeita.`)) return;
    setBusy(true);
    try { await apiDelete(`/api/queues/${queueId}/promotions/${promotion.id}`); await onChanged(); }
