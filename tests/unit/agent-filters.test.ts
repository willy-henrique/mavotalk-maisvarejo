...
test("monitoramento de agentes filtra instalações no servidor e no tenant atual", async () => {
  const [repository, route, systemStatusRoute, view] = await Promise.all([
    readFile("lib/agent-cloud/agent-repository.ts", "utf8"),
    readFile("app/api/admin/agents/route.ts", "utf8"),
    readFile("app/api/admin/system-status/route.ts", "utf8"),
  ...
  assert.match(systemStatusRoute, /queryTenantDatabase/);
  assert.match(systemStatusRoute, /auth\.session\.organizationId/);
  assert.doesNotMatch(systemStatusRoute, /queryDatabase/);
