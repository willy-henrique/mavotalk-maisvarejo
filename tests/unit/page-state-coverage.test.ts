...
test("estados de página compartilhados cobrem áreas administrativas e operacionais", async () => {
  const [states, analytics, agents, quickReplies, businessAccess, contacts] = await Promise.all([
  ...
  for (const source of [analytics, agents, quickReplies, businessAccess, contacts]) {
