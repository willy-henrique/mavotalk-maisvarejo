...
| Resolvido | Checkboxes nativos e ambiguidade entre menu e permissão | O Menu do painel usa switches com nome acessível, tabs navegáveis por teclado e painéis separados para visibilidade e matriz de ações; a própria tela esclarece que esconder navegação não substitui o RBAC do servidor. |
...
| P2 | Primitives compartilhadas incompletas | Tokens, `Dialog`, paginação, páginas, cards, botões, campos, estados de loading/vazio/erro e `StatusBadge` foram centralizados; Contatos, Filas, Auditoria, Acessos, Agentes e Equipe usam o badge compartilhado. Respostas rápidas possui prévia local de variáveis, mas suas tabelas e metadados ainda precisam ser enriquecidos com dados que a API atual não expõe. |
| P2 | Fluxos administrativos têm feedback desigual | Dashboard, Central de conexão, Menu do painel, Equipe e Filas expõem loading e falhas de carregamento com tentativa novamente; outras ações ainda não oferecem confirmação, prevenção uniforme de duplo envio, confirmação destrutiva ou aviso de alterações não salvas. |
