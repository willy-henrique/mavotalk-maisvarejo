...
| P2 | Master está limitado à organização padrão | rotas master usam `DEFAULT_ORG_ID`; não há seletor/escopo explícito de organização no painel | modelar autorização de plataforma, seleção de tenant auditada e barreira de impersonation; até lá documentar que master não é console multiempresa geral. |
