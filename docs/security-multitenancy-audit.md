...
| P1 | RLS ainda depende de comprovação de infraestrutura | Repositório operacional, Inbox, contatos, métricas e mutações administrativas agora executam com `app.organization_id` via adaptador tenant-aware. A efetividade final ainda depende de o papel de conexão não ser owner/BYPASSRLS e de as policies/migrations terem sido aplicadas no banco QA/produção. | usar papel de aplicação sem `BYPASSRLS`, considerar `FORCE ROW LEVEL SECURITY` após validar os fluxos de plataforma e executar prova negativa A→B no banco QA. |
...
| Resolvido | Webhook e ação manual concorrentes podiam criar conversas/tickets duplicados | A rotina fazia leitura e dois inserts independentes pelo adaptador HTTP, sem transação ou bloqueio por contato. | A rotina agora usa uma transação RLS do tenant, advisory lock por organização/contato, pesquisa bloqueante e inserts atômicos de conversa e ticket. |
| Resolvido | Filas BullMQ sem processamento real aparentavam confiabilidade inexistente | `webhooks` e `agent-sync` não tinham produtor e seus workers apenas registravam logs. | Filas e workers mortos foram removidos. Permanecem SLA, com produtor e revalidação transacional, e limpeza de mídia, com produtor Cloudinary. |
