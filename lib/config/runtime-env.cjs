...
const apiProductionSchema = commonProductionSchema
  ...
  .superRefine((environment, context) => {
    ...
    if ((environment.WHATSAPP_PROVIDER || "unofficial") === "unofficial") {
      ...
      // Instâncias gratuitas do Render não aceitam disco persistente. O opt-in
      // explícito abaixo permite subir sem disco, aceitando que a sessão do
      // WhatsApp é perdida a cada restart e o QR precisa ser lido de novo.
