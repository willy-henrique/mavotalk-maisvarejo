...
const apiProductionSchema = commonProductionSchema
  ...
  .superRefine((environment, context) => {
    const whatsappProvider = String(
      environment.WHATSAPP_PROVIDER || "unofficial",
    ).toLowerCase();
    if (!["unofficial", "twilio"].includes(whatsappProvider)) {
      context.addIssue({
        code: "custom",
        path: ["WHATSAPP_PROVIDER"],
        message: "deve ser 'unofficial' ou 'twilio'",
      });
    }
    ...
    if (whatsappProvider === "unofficial") {
